...
function SkipStopDialog({ orderNumber, note, setNote, completing, onCancel, onConfirm }: { orderNumber: string; note: string; setNote: (value: string) => void; completing: boolean; onCancel: () => void; onConfirm: () => void }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#171126]/60 p-4 backdrop-blur-sm"><form role="dialog" aria-modal="true" aria-labelledby="skip-stop-title" onSubmit={(event) => { event.preventDefault(); if (note.trim().length >= 3) onConfirm(); }} className="w-full max-w-md rounded-[22px] bg-[#fffdfb] p-5 shadow-2xl"><p className="text-[10px] font-extrabold uppercase tracking-[.15em] text-rose-700">Route exception</p><h2 id="skip-stop-title" className="mt-1 text-xl font-semibold text-[#302547]">Skip {orderNumber}?</h2><p className="mt-1 text-sm text-[#736c82]">Add a short reason. It is saved with the route handoff audit.</p><label className="mt-4 block text-[10px] font-extrabold uppercase tracking-[.12em] text-[#746c84]">Reason<textarea autoFocus aria-label="Skip reason" value={note} onChange={(event) => setNote(event.target.value)} minLength={3} className="mt-1.5 min-h-24 w-full rounded-xl border border-[#26203f]/15 bg-white px-3 py-2 text-sm font-normal normal-case tracking-normal outline-none focus:ring-2 focus:ring-brand-500" /></label><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onCancel} className="rounded-xl border border-[#26203f]/12 px-3 py-2 text-sm font-semibold text-[#6f687d]">Keep stop</button><button type="submit" disabled={completing || note.trim().length < 3} className="rounded-xl bg-rose-700 px-3 py-2 text-sm font-bold text-white disabled:opacity-50">Confirm skip</button></div></form></div>;
}
